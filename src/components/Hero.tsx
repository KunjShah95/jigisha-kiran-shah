...
const Hero = () => {
  return (
    <section className="hero relative overflow-hidden min-h-[95vh] flex items-center bg-slate-50" id="home">
      ...
      <div className="container relative z-10 w-full pt-20">
        <div className="grid lg:grid-cols-12 gap-12 lg:gap-16 items-center">
          {/* Content Column */}
          <div className="lg:col-span-7 hero__content reveal">
            <div className="inline-flex items-center gap-2 px-6 py-2.5 rounded-full bg-white border border-gold/30 shadow-md mb-8 hover:shadow-lg transition-shadow duration-300">
              <Award className="w-5 h-5 text-gold fill-gold" />
              <span className="text-sm font-bold text-midnight tracking-wide uppercase">Double MDRT (USA) Achiever</span>
            </div>

            <h1 className="text-5xl lg:text-7xl font-playfair font-bold text-midnight leading-[1.1] mb-8 tracking-tight">
              Safeguarding Your <br />
              <span className="text-gradient-gold italic relative inline-block">
                Wealth & Legacy
                <svg className="absolute w-full h-3 -bottom-2 left-0 text-gold/30" viewBox="0 0 100 10" preserveAspectRatio="none">
                  <path d="M0 5 Q 50 10 100 5" stroke="currentColor" strokeWidth="3" fill="none" />
                </svg>
              </span>
            </h1>

            <p className="text-lg lg:text-xl text-gray-600 mb-10 max-w-xl leading-relaxed font-light">
            ...
            {/* Trust Indicators */}
            <div className="grid grid-cols-2 md:grid-cols-4 gap-8 py-8 border-t border-gray-200/60 w-full">
              <div className="reveal-scale group cursor-default" style={{ transitionDelay: '0.2s' }}>
                <div className="flex items-center gap-2 mb-1">
                  <Shield className="w-5 h-5 text-gold" />
                  <div className="text-3xl font-bold text-midnight group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-linear-to-r group-hover:from-midnight group-hover:to-blue-600 transition-all">22+</div>
                </div>
                <div className="text-xs text-gray-500 uppercase tracking-widest font-semibold">Years Exp.</div>
              </div>
              <div className="reveal-scale group cursor-default" style={{ transitionDelay: '0.4s' }}>
                <div className="flex items-center gap-2 mb-1">
                  <Users className="w-5 h-5 text-gold" />
                  <div className="text-3xl font-bold text-midnight group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-linear-to-r group-hover:from-midnight group-hover:to-blue-600 transition-all">2.7k+</div>
                </div>
                <div className="text-xs text-gray-500 uppercase tracking-widest font-semibold">Families</div>
              </div>
               <div className="reveal-scale group cursor-default" style={{ transitionDelay: '0.6s' }}>
                <div className="flex items-center gap-2 mb-1">
                  <Star className="w-5 h-5 text-gold fill-gold" />
                  <div className="text-3xl font-bold text-midnight">5.0</div>
                </div>
                <div className="text-xs text-gray-500 uppercase tracking-widest font-semibold">Google Rate</div>
              </div>
              <div className="reveal-scale group cursor-default" style={{ transitionDelay: '0.8s' }}>
                <div className="flex items-center gap-2 mb-1">
                  <Award className="w-5 h-5 text-gold" />
                  <div className="text-3xl font-bold text-midnight">Double</div>
                </div>
                <div className="text-xs text-gray-500 uppercase tracking-widest font-semibold">MDRT USA</div>
              </div>
            </div>
          </div>

          {/* Visual Column */}
          <div className="lg:col-span-5 relative mt-12 lg:mt-0 reveal-right hidden lg:block">
            <div className="relative w-full aspect-4/5 max-w-md mx-auto">
              ...
              <div className="relative h-full w-full rounded-4xl overflow-hidden shadow-2xl z-10 group">
                ...
                <div className="absolute bottom-0 left-0 right-0 p-8 text-white transform translate-y-2 group-hover:translate-y-0 transition-transform duration-500">
                  ...
                  <div className="font-playfair text-3xl font-bold mb-1">Jigisha Kiran Shah</div>
                  <p className="text-white/80 text-sm font-medium tracking-wide">Expert Financial guidance for 2 decades</p>
                </div>
              </div>

              {/* Floating Trust Badge */}
              <div className="absolute -top-6 -right-6 bg-white/95 backdrop-blur-md p-5 rounded-xl shadow-xl border border-white/40 max-w-[200px] animate-float z-20">
